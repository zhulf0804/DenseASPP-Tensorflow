...
def dense_block_layer(input, k, k0, layer, train, keep_prob, rate):
    '''
    ...
    with tf.name_scope("dense_bottleneck_layer"):
        ...
        input = bn_layer(input, train)
        input = tf.nn.relu(input)
        input = tf.nn.atrous_conv2d(input, weights_3, rate, padding='SAME', name='conv3')

    return input


def dense_block(input, k, layers_num, train, keep_prob, rate):
    '''
    dense block
    :param input: input feature map
    :param k: output channels
    ...
    for i in range(1, layers_num + 1):
        ...
        with tf.name_scope("layer_%d" % i):
            output = dense_block_layer(input, k, k0, i, train, keep_prob, rate)
            output = tf.nn.dropout(output, keep_prob=keep_prob)
            input = tf.concat(values=[input, output], axis=-1)
    return input

def transition_layer(input, train, pool=True):
    """
        ...
        for i in range(1, 1 + dense_blocks_num):
            with tf.name_scope("dense_block_%d" % i):
                if i == dense_blocks_num:
                    input = dense_block(input, k, layers[i - 1], train, keep_prob, rate=4)
                elif i == dense_blocks_num - 1:
                    input = dense_block(input, k, layers[i - 1], train, keep_prob, rate=2)
                else:
                    input = dense_block(input, k, layers[i - 1], train, keep_prob, rate=1)

                if i == 1:
                    input = transition_layer(input, train, pool=True)
                else:
                    input = transition_layer(input, train, pool=False)
...
if __name__ == '__main__':
    ...
    output = densenet_121(input, keep_prob=0.9, train=True)
    print(output)