...
image_batch, anno_batch, filename = input_data.read_batch(BATCH_SIZE, type=prediction_on, shuffle=False)
...
#weights = tf.cast(tf.not_equal(y, CLASSES), tf.int32)
#mIoU, update_op = tf.metrics.mean_iou(labels=y, predictions=prediction, num_classes=CLASSES, weights=weights)
...
with tf.Session() as sess:
    ...
    saver.restore(sess, './checkpoint/denseASPP.model-16000')
    ...
    print("mIoU is %f" % np.mean(mIoU_val_all))
